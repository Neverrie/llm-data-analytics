...
### Почему это не «абсолютная» безопасность
Sandbox сильно снижает риск, но не заменяет полноценную изоляцию уровня VM/контейнера с seccomp/AppArmor и сетевыми policy. Для production рекомендуется:
- запуск sandbox-worker в отдельном изолированном контейнере/namespace,
- `read-only` rootfs + ограниченные volume mounts,
- жесткие cgroup-лимиты CPU/RAM,
- egress deny-by-default,
- аудит логов и алерты на аномальные попытки.

### Практические правила для разработчиков
- Не выполнять код модели вне sandbox.
- Не расширять allowlist библиотек без ревью.
- Не отдавать пользователю сырые технические trace-блоки в основном UI.
- Любой новый тип артефакта сначала регистрировать в backend, потом рендерить в клиенте.
- Держать `.env` вне репозитория и регулярно ротировать API-ключи.
