...
Цикл имеет жесткий технический предел в 30 шагов. Агент сам определяет необходимое число действий по сложности задачи и не должен завершать анализ до получения достаточных результатов. Каждый запуск использует единый рабочий каталог, поэтому следующие действия могут продолжать предыдущие и обновлять созданные файлы.
